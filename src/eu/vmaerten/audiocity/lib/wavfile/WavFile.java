...
package eu.vmaerten.audiocity.lib.wavfile;
...
public class WavFile
{
	...
	public static WavFile newWavFile(File file, int numChannels, long numFrames, int validBits, long sampleRate) throws IOException, WavFileException
	{
		...
		long mainChunkSize =	4 +	// Riff Type
									8 +	// Format ID and getSize
									16 +	// Format data
									8 + 	// Data ID and getSize
									dataChunkSize;

		// Chunks must be word aligned, so if odd number of audio data bytes
		// adjust the main chunk getSize
	...
	public static WavFile openWavFile(File file) throws IOException, WavFileException
	{
		...
		// Check that the file getSize matches the number of bytes listed in header
		if (file.length() != chunkSize+8) {
			throw new WavFileException("Header chunk getSize (" + chunkSize + ") does not match file getSize (" + file.length() + ")");
		}
		...
		while (true)
		{
			// Read the first 8 bytes of the chunk (ID and chunk getSize)
			...
			else if (chunkID == DATA_CHUNK_ID)
			{
				...
				if (chunkSize % wavFile.blockAlign != 0) throw new WavFileException("Data Chunk getSize is not multiple of Block Align");
